...
const washPrint = (name, settings, placeList) => {
  ...
  <div class="w-full px-5 pb-5">
    ...
    Автомойка
      ...
      <div class="flex justify-between text-xs mb-1 pb-2 border-b-2 border-black">
        <p>Номер заказа:</p>
        <p class="text-base">
          ${name.id_washs ? `№ <b>${name.id_washs}</b>` : ""}
        </p>
      </div>
      <div class="flex justify-between text-xs">
        <p>Дата:</p>
  ...
  fs.writeFile(app.getPath("userData") + "/wash.html", check, (err) => {
    if (err) {
      console.log("Error writing wash html", err);
    } else {
      console.log("Successfully wrote wash html");
    }
  });
};

module.exports = washPrint;
