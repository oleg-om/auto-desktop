...
const windowPrint = (name, settings, placeList) => {
  ...
  <div class="w-full px-5 pb-5">
      ...
      <div class="flex justify-between text-xs mb-1 pb-2 border-b-2 border-black">
        <p>Номер заказа:</p>
        <p class="text-base">
          ${name.id_windows ? `№ <b>${name.id_windows}</b>` : ""}
        </p>
      </div>
      <div class="flex justify-between text-xs">
        <p>Дата:</p>
  ...
  fs.writeFile(app.getPath("userData") + "/window.html", check, (err) => {
    if (err) {
      console.log("Error writing window html", err);
    } else {
      console.log("Successfully wrote window html");
    }
  });
};

module.exports = windowPrint;
